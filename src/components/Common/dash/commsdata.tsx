...
const CommsData: React.FC = () => {
  ...
  return (
    <div className="bg-white text-black rounded-sm flex flex-col flex-grow w-1/2">
      <div className="border-b border-gray-200 text-sm p-1 uppercase font-semibold flex items-center"> 
        Transmission <span className="text-[10px] font-light ml-1">X-band (7-11.2 GHz)</span>
      </div>

      <div className="grid grid-cols-2 h-1/2 border-b border-gray-200">
        <div className="flex flex-col justify-center items-center border-r border-gray-200 p-2">
          <div className="text-xs font-semibold">Uplink</div>
          <div className="text-2xl font-bold flex flex-col items-center">
            {commsData.uplink.toFixed(2)} 
            <span className="text-xs font-normal">Mbps</span>
          </div>
        </div>
        <div className="flex flex-col justify-center items-center p-2">
          <div className="text-xs font-semibold">Downlink</div>
          <div className="text-2xl font-bold flex flex-col items-center">
            {commsData.downlink.toFixed(2)} 
            <span className="text-xs font-normal">Mbps</span>
          </div>
        </div>
      </div>

      <div className="flex flex-row flex-grow text-center text-xs h-1/2">
        <div className="border-r p-1 flex-grow w-1/3 flex flex-col justify-center items-center">
          <div className="font-semibold">End-to-End Latency</div>
          <div>{commsData.latency.toFixed(2)} ms</div>
        </div>
        <div className="border-r p-1 w-1/3 flex flex-col justify-center items-center">
          <div className="font-semibold">Signal Strength</div>
          <div>{commsData.signalStrength.toFixed(2)} dBm</div>
        </div>
        <div className="p-1 w-1/3 flex flex-col justify-center items-center">
