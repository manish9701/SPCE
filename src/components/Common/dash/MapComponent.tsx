...
interface MapComponentProps {
  ...
  mapStyle?: 'detailed' | 'simple';
}

const MapComponent: React.FC<MapComponentProps> = ({ satData, mapStyle = 'detailed' }) => {
  ...
  useEffect(() => {
    if (mapContainerRef.current && !mapRef.current) {
      if (mapStyle === 'detailed') {
        ...
        const tileLayer = L.tileLayer('https://tiles.stadiamaps.com/tiles/stamen_toner_background/{z}/{x}/{y}{r}.png', {
          minZoom: 0,
          maxZoom: 20,
          width: 100,
        } as any).addTo(mapRef.current);
